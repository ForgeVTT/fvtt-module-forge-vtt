...
/* global ui, Actor, AudioContainer, CONFIG, CONST, Dialog, Entity, FilePicker, ForgeAssetSyncApp, FormApplication, foundry, game, Hooks, isNewerVersion, MESSAGES, Module, ModuleManagement, setProperty, Setup, TextureLoader, TokenDocument, preparePostOverride */
...
class ForgeVTT {
        ...
        if (this.usingTheForge) {
            ...
            if (window.location.pathname.startsWith("/join")) {
                ...
                Hooks.on("renderSettings", (obj, html) => {
                    ...
                    const join = ForgeVTT.ensureIsJQuery(html).find(
                        "button:is([data-action='logout'], [data-app='logout'])"
                    );
                    ...
                    if (ForgeAPI.lastStatus && !ForgeAPI.lastStatus.table) {
                        ForgeVTT.ensureIsJQuery(html)
                            .find("button:is([data-action='setup'], [data-app='setup'])")
                            .hide();
                    }
                });
                ...
                Hooks.on("preUpdateActor", (actor, changed) => {
                    ...
                    switch (systemId) {
                        case "pf2e":
                            ...
                            [Actor.DEFAULT_ICON, `systems/pf2e/icons/default-icons/${actor.type}.svg`].forEach(
                                (img) => {
                    ...
                    if (ForgeCompatibility.isNewerVersion(ForgeVTT.foundryVersion, "10")) {
                        if (!changed.prototypeToken?.texture?.src) {
                            if (
                                ...
                                defaultTokenImages.includes(actor.prototypeToken?.texture?.src)
                            ) {
                    ...
                    case "pf2e": {
                        ...
                        const replaceDefaultIcon = function () {
                            try {
                                ...
                                const defaultIcons = [];
                                [Actor.DEFAULT_ICON, `systems/pf2e/icons/default-icons/${this.actor.type}.svg`].forEach(
                                    (img) => {
                                        defaultIcons.push(img);
                                        // The Bazaar uses an 'assets' folder on the top level of the package to store media assets
                                        defaultIcons.push(
                                            `bazaar/${img.replace("systems/pf2e/", "systems/pf2e/assets/")}`
                                        );
                                    }
                                );
    ...
    static i18nInit() {
        ...
        if (game.i18n.has("THEFORGE.LoadingWorldDataTroubleshoot")) {
            $("#forge-loading-progress .loading-warning").html(
                game.i18n.localize("THEFORGE.LoadingWorldDataTroubleshoot")
            );
        }
    }
    ...
    static animateProgress() {
        if (!$("#forge-loading-progress").hasClass("slow")) {
            ...
            $("#forge-loading-progress .loading-progress .bar")
                ...
                .animate(
                    { width: "100%" },
                    { duration: duration, easing: "linear", complete: ForgeVTT.animateProgress }
                );
        }
    }
    ...
    /**
     * Retrieves a Livekit access token for the specified room and user.
     ...
     * @returns {Promise<string>} A promise that resolves to the Livekit access token, or an empty string if an error occurs.
     */
    ...
    static async _verifyInactivePlayer() {
        ...
        if (inactiveFor > inactiveThreshold) {
            await ForgeAPI.call(null, {
                action: "inactive",
                path: window.location.pathname,
                inactivity: inactiveFor,
            }).catch(console.error);
    ...
    static async _sendHeartBeat(initial) {
        ...
        const response =
            (await ForgeAPI.call(null, { action: "heartbeat", active, initial }).catch(console.error)) || {};
     ...
     * @param {string} entityType - The type of entity being processed (e.g., "Actor", "Token", "Scene").
     * @param {object} data - The data object containing information to process.
     * @returns {Promise} A promise that resolves after processing the data images.
     */
        ...
        switch (entityType) {
            case "Actor":
                ...
                if (data.items) {
                    data.items = await Promise.all(
                        data.items.map((item) => this.findAndDestroyDataImages("Item", item))
                    );
                }
            ...
            case "Item":
                ...
                if (data.system?.description?.value) {
                    data.system.description.value = await this._migrateDataImageInHTML(
                        entityType,
                        data.system.description.value
                    );
                } else if (!data.system && data.data?.description?.value) {
                    data.data.description.value = await this._migrateDataImageInHTML(
                        entityType,
            ...
            case "Scene":
                ...
                if (data.notes) {
                    data.notes = await Promise.all(
                        data.notes.map((note) => this.findAndDestroyDataImages("Note", note))
                    );
                }
                if (data.templates) {
                ...
                if (data.tiles) {
                    data.tiles = await Promise.all(
                        data.tiles.map((tile) => this.findAndDestroyDataImages("Tile", tile))
                    );
                }
                if (data.tokens) {
                    data.tokens = await Promise.all(
                        data.tokens.map((token) => this.findAndDestroyDataImages("Token", token))
                    );
                }
                break;
            case "Drawing":
     ...
     * @param {string} entityType - The type of entity being processed (e.g., "Actor", "Token", "Scene").
     * @param {string} img - The image URL or data URL to be uploaded.
     * @returns {Promise<string>} A promise that resolves to the new URL or the original string if upload fails.
     */
     ...
     * @returns {object} An object containing Forge-specific messages mapped by translation keys.
     */
...
class ForgeVTT_FilePicker extends FilePicker {
     ...
     * @returns {Promise<Array<object>>} A promise that resolves to an array of Forge VTT bucket objects.
     */
    ...
    async _render(...args) {
        ...
        if (this._newFilePicker) {
            ...
            if (this.activeSource === "forgevtt") {
                ...
                uploadDiv.find('button[name="forgevtt-upload"]').on("click", () => upload.trigger("click"));
                uploadDiv.find('button[name="forgevtt-new-folder"]').on("click", () => this._onNewFolder());
            }
        }
                ...
                ok: {
                    ...
                    callback: async (html) => {
                        ...
                        if (!response || response.error) {
                            ui.notifications.error(
                                response ? response.error : "An unknown error occurred accessing The Forge API"
                            );
     ...
     * @returns {Promise<object|boolean>} A promise that resolves to the upload result or false if the upload fails.
     */
